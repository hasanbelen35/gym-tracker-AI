...
export default function GymLogin() {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center relative overflow-hidden bg-[#EEF2FF] py-10" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <button onClick={() => router.push(CONFIG.backPath)} className="absolute top-5 left-5 bg-white/80 border border-[#D8DCEE] rounded-lg px-3 py-1.5 text-xs font-medium text-[#5B6FD4] hover:bg-white transition flex items-center gap-1.5">
        ← Geri
      </button>
      ...
      <form onSubmit={handleLogin} className="bg-white rounded-2xl p-10 w-full max-w-sm shadow-[0_4px_40px_rgba(91,111,212,0.08)] relative z-10">
        ...
        <h1 className="text-2xl font-extrabold text-[#1A1A2E] mb-1" style={{ fontFamily: "'Syne', sans-serif" }}>{CONFIG.title}</h1>
        <p className="text-sm text-[#9A9AB0] mb-7">{CONFIG.subtitle}</p>
        ...
        <p className="text-center text-xs text-[#AAAFDB] mt-5">
          Hesabınız yok mu? <span onClick={() => router.push(CONFIG.registerPath)} className="text-[#5B6FD4] font-medium cursor-pointer hover:underline">Hemen Kayıt Ol</span>
        </p>
      </form>
    </main>
  );
