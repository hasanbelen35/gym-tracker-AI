...
const IconUserX = ({ className = "" }: { className?: string }) => (
  <svg viewBox="0 0 24 24" fill="none" className={className} xmlns="http://www.w3.org/2000/svg">
    <circle cx="9" cy="8" r="3.5" stroke="currentColor" strokeWidth="2" />
    <path d="M3.5 19c.8-3.2 3-4.8 5.5-4.8s4.7 1.6 5.5 4.8" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
...
export default function TrainerAthletesPage() {
    ...
    const handleRequest = (memberPublicId: string) => {
        if (!gymId) return;
        dispatch(requestAssignment(memberPublicId))
            .unwrap()
            .then(() => {
                dispatch(fetchMembersByStatus({ gymId, status: 'UNASSIGNED' }));
                dispatch(fetchMembersByStatus({ gymId, status: 'PENDING' }));
    ...
    return (
        <div className="min-h-screen bg-[var(--background)] py-10 font-sans text-[var(--foreground)]">
            <div className="mx-auto max-w-6xl px-4 sm:px-6">
                ...
                {/* PIPELINE STRIP — reflects the real member lifecycle */}
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                    {stages.map((stage) => {
                        ...
                        return (
                            ...
                            >
                                {/* Column header — stenciled plate style */}
                                    ...
                                    {loading && stage.members.length === 0 ? (
                                        ...
                                        stage.members.map((m: Member) => (
                                            <button
                                                ...
                                                className="group flex w-full items-center justify-between gap-3 rounded-lg border border-nav-border bg-[var(--background)] p-3 text-left transition-colors hover:border-brand-500/50"
                                            >
                                                <div className="flex items-center gap-3">
                                        ...
                                        stage.members.map((m: Member) => (
                                            ...
                                            >
                                                ...
                                                <button
                                                    ...
                                                    className="flex flex-none items-center gap-1 rounded-lg border border-nav-border px-2.5 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-[var(--foreground)]/55 transition-colors hover:border-brand-600/40 hover:text-brand-text"
                                                >
